...
// --- HELPERS ---
function createInput(val = "", ph = "") { const i = document.createElement("input"); i.value = val; i.placeholder = ph; return i; }
function createSelect(arr, val) { const s = document.createElement("select"); arr.forEach(g => { const o = document.createElement("option"); o.value = o.innerText = g; if (g === val) o.selected = true; s.appendChild(o); }); return s; }
function avg(g, votes = []) { if (!votes.length) return g; const all = [g, ...votes].map(x => grades.indexOf(x)).sort((a, b) => a - b); return grades[all[Math.floor(all.length / 2)]]; }
function median(g, votes) { return avg(g, votes); }
